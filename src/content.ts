...
function showSelectionButton(text: string, rect: DOMRect, point: { x: number; y: number }): void {
  ...
  const shadow = host.attachShadow({ mode: "closed" });
  shadow.innerHTML = `<style>:host { all: initial; position: fixed; z-index: 2147483647; } button { width: 32px; height: 32px; border: 1px solid rgba(32,76,61,.16); border-radius: 27%; color: #b7e8ce; background: #153b31; box-shadow: 0 6px 18px rgba(17,45,36,.22); cursor: pointer; font: 700 21.333333px/1 "PingFang SC","Microsoft YaHei",sans-serif; }</style><button title="翻译选中文本" aria-label="翻译选中文本">译</button>`;
